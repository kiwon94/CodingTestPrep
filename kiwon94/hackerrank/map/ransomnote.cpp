...
void checkMagazine(vector<string> magazine, vector<string> note) {
    ...
    {
        auto itr = n_map.find(note[i]);
        if (itr == n_map.end())
        {
            n_map.insert({ note[i],1 });
        }
        else
        {
            itr->second++;
    ...
    for(int i=0;i<note.size();i++)
    {
        if(m_map.find(note[i])==m_map.end())
        {
            chk=false;
            break;
        }
        ...
        {
            auto m_iter=m_map.find(note[i]);
            auto n_iter=n_map.find(note[i]);
            int m_cnt=m_iter->second;
            int n_cnt=n_iter->second;
            if(m_cnt<n_cnt)
            {
                chk=false;
                break;
